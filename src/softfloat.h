/* This source is compiled by 4c, never by the host compiler. SF preserves a
...
SF(unsigned long __4c_sf_shr1(unsigned long value);)
SF(unsigned long __4c_sf_shr(unsigned long value, int count) {
    if (count >= 64) return 0UL;
    while (count > 0) { value=__4c_sf_shr1(value); count-=1; }
    return value;
})
SF(unsigned long __4c_sf_jam(unsigned long value, int count) {
    if (count >= 64) { if (value) return 1UL; return 0UL; }
    while (count > 0) {
        unsigned long next=__4c_sf_shr1(value);
        if (value != next+next) {
            unsigned long half=__4c_sf_shr1(next);
...
SF(unsigned long __4c_sf_pack(unsigned long sign, int exponent, unsigned long ext) {
    if (!ext) return sign;
    while (ext >= 72057594037927936UL) {
        ext=__4c_sf_jam(ext,1); exponent+=1;
    }
    while (ext < 36028797018963968UL) { ext=ext+ext; exponent-=1; }
    if (exponent < -1022) {
        ext=__4c_sf_jam(ext,-1022-exponent); exponent=-1022;
    }
    unsigned long rounded=__4c_sf_shr(ext,3);
    unsigned long tail=ext-rounded*8UL;
    unsigned long half=__4c_sf_shr1(rounded);
    if (tail > 4UL || (tail == 4UL && rounded != half+half)) rounded+=1;
    if (rounded >= 9007199254740992UL) {
        rounded=__4c_sf_shr1(rounded); exponent+=1;
    }
...
SF(unsigned long __4c_sf_u64(unsigned long value) {
    return __4c_sf_pack(0UL,55,value);
})
SF(unsigned long __4c_sf_i64(long value) {
    ...
    return __4c_sf_pack(sign,55,magnitude);
})
...
SF(unsigned long __4c_sf_binary(unsigned long a, unsigned long b, int op) {
    unsigned long sign=0UL;
    ...
    int ea=(int)__4c_sf_shr(a,52);
    int eb=(int)__4c_sf_shr(b,52);
    ...
    if (op) {
        ...
        unsigned long half=__4c_sf_shr1(quotient);
        if (ma && quotient == half+half) quotient+=1;
        return __4c_sf_pack(sign,ea-eb,quotient);
    }
    unsigned long hi=0UL, lo=0UL, ahi=0UL;
    int i=0;
    while (i < 53) {
        unsigned long half=__4c_sf_shr1(mb);
        if (mb != half+half) {
    ...
    unsigned long lowtop=__4c_sf_shr(lo,49);
    unsigned long ext=hi*32768UL+lowtop;
    unsigned long half=__4c_sf_shr1(ext);
    if (lo != lowtop*562949953421312UL && ext == half+half) ext+=1;
    return __4c_sf_pack(sign,ea+eb,ext);
})
};
#undef SF
